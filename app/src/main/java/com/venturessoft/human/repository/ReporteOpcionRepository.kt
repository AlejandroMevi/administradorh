...
class ReporteOpcionRepository : ValidateAndRefreshJWTRepository() {
    ...
    fun getReporteOpcion(reporteOpcionRequest: ReporteOpcionRequest, responseLiveData: MutableLiveData<ReporteOpcionResponse>, isLoading: MutableLiveData<Boolean>, codeError: MutableLiveData<Int>) {
        ...
        validateAndRefreshJWT(callBackJWTValidateAndRefresh = object : CallBackJWTValidateAndRefresh {
            ...
            override fun onSuccess() {
                ...
                retrofitClient.getReportesOpcion(reporteOpcionRequest, User.token).enqueue(

                    object : Callback<ReporteOpcionResponse> {

                        override fun onFailure(call: Call<ReporteOpcionResponse>?, t: Throwable?) {
                            sendCustomEventCatch(t, EventVariableFirebase.EVENT_WS_ADMDES_FAILURE_ALTA_ADMINISTRAR_COMPRA)
                            isLoading.postValue(false)
                            codeError.postValue(600)
                        }

                        override fun onResponse(call: Call<ReporteOpcionResponse>?, response: Response<ReporteOpcionResponse>?) {
                            if (response!!.isSuccessful) {
                                isLoading.postValue(false)
                                responseLiveData.postValue(response.body())
                                response.body().codigo
                            } else if(response.code() == HttpURLConnection.HTTP_FORBIDDEN) {
                                getReporteOpcion(reporteOpcionRequest, responseLiveData, isLoading, codeError)
                            } else {
