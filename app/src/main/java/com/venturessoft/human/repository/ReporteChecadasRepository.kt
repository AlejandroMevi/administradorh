...
class ReporteChecadasRepository : ValidateAndRefreshJWTRepository() {
    ...
    fun getReporteChecadas(reporteChecadasRequest: ReporteChecadasRequest, responseLiveData: MutableLiveData<ReporteChecadasResponse>, isLoading: MutableLiveData<Boolean>, codeError: MutableLiveData<Int>) {
        ...
        validateAndRefreshJWT(callBackJWTValidateAndRefresh = object : CallBackJWTValidateAndRefresh {
            ...
            override fun onSuccess() {
                ...
                retrofitClient.getReporteChecadas(reporteChecadasRequest, User.token).enqueue(

                    object : Callback<ReporteChecadasResponse> {
                        override fun onFailure(call: Call<ReporteChecadasResponse>?, t: Throwable?) {
                            sendCustomEventCatch(t, EventVariableFirebase.EVENT_WS_ADMDES_FAILURE_ALTA_ADMINISTRAR_COMPRA)
                            isLoading.postValue(false)
                            codeError.postValue(600)
                        }

                        override fun onResponse(call: Call<ReporteChecadasResponse>?, response: Response<ReporteChecadasResponse>?) {
                            if (response!!.isSuccessful) {
                                isLoading.postValue(false)
                                responseLiveData.postValue(response.body())
                                response.body().codigo
                            } else if(response.code() == HttpURLConnection.HTTP_FORBIDDEN) {
                                getReporteChecadas(reporteChecadasRequest, responseLiveData, isLoading, codeError)
                            } else {
