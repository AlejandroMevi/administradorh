...
class ActualizaZonaRepository : ValidateAndRefreshJWTRepository() {
    ...
    fun getActualizaZona(actualizaZonaRequest: ActualizaZonaRequest, responseLiveData: MutableLiveData<ActualizaZonaResponse>, isLoading: MutableLiveData<Boolean>, codeError: MutableLiveData<Int>) {
        isLoading.postValue(true)
        validateAndRefreshJWT(callBackJWTValidateAndRefresh = object :
            CallBackJWTValidateAndRefresh {

            override fun onSuccess() {
                retrofitClient.getActualizaZonas(actualizaZonaRequest, User.token).enqueue(

                    object : Callback<ActualizaZonaResponse> {
                        override fun onFailure(call: Call<ActualizaZonaResponse>?, t: Throwable?) {
                            sendCustomEventCatch(t, EventVariableFirebase.EVENT_WS_ADMDES_FAILURE_ALTA_ADMINISTRAR_COMPRA)
                            isLoading.postValue(false)
                            codeError.postValue(600)
                        }

                        override fun onResponse(call: Call<ActualizaZonaResponse>?, response: Response<ActualizaZonaResponse>?) {
                            if (response!!.isSuccessful) {
                                isLoading.postValue(false)
                                responseLiveData.postValue(response.body())
                                response.body().codigo
                            } else if (response.code() == HttpURLConnection.HTTP_FORBIDDEN) {
                                getActualizaZona(actualizaZonaRequest, responseLiveData, isLoading, codeError)
                            } else {
