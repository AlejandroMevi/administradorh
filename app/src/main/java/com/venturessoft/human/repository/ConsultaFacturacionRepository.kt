...
class ConsultaFacturacionRepository : ValidateAndRefreshJWTRepository() {
    ...
    fun getConsultaFacturacion(consultaFacturacionRequest: ConsultaFacturacionRequest, responseLiveData: MutableLiveData<ConsultaFacturacionResponse>, isLoading: MutableLiveData<Boolean>, codeError: MutableLiveData<Int>) {
        ...
        validateAndRefreshJWT(callBackJWTValidateAndRefresh = object : CallBackJWTValidateAndRefresh {
            ...
            override fun onSuccess() {
                ...
                retrofitClient.getConsultaFacturacion(User.scia!![0].cia, User.scia!![0].idFacturacion, User.token).enqueue(

                    object : Callback<ConsultaFacturacionResponse> {
                        override fun onFailure(call: Call<ConsultaFacturacionResponse>?, t: Throwable?) {
                            sendCustomEventCatch(t, EventVariableFirebase.EVENT_WS_ADMDES_FAILURE_ALTA_ADMINISTRAR_COMPRA)
                            isLoading.postValue(false)
                            codeError.postValue(600)
                        }

                        override fun onResponse(call: Call<ConsultaFacturacionResponse>?, response: Response<ConsultaFacturacionResponse>?) {
                            if (response!!.isSuccessful) {
                                isLoading.postValue(false)
                                responseLiveData.postValue(response.body())
                                response.body().codigo
                            } else if(response.code() == HttpURLConnection.HTTP_FORBIDDEN) {
                                getConsultaFacturacion(consultaFacturacionRequest, responseLiveData, isLoading, codeError)
                            } else {
