...
class EliminaEmpleadoRepository : ValidateAndRefreshJWTRepository() {
    ...
    fun getEliminaEmpleado(eliminaEmpleadoRequest: EliminaEmpleadoRequest, responseLiveData: MutableLiveData<EliminaEmpleadoResponse>, isLoading: MutableLiveData<Boolean>, codeError: MutableLiveData<Int>) {
        ...
        validateAndRefreshJWT(callBackJWTValidateAndRefresh = object : CallBackJWTValidateAndRefresh {
            ...
            override fun onSuccess() {
                ...
                retrofitClient.getEliminaEmpleado(eliminaEmpleadoRequest, User.token).enqueue(

                    object : Callback<EliminaEmpleadoResponse> {

                        override fun onFailure(call: Call<EliminaEmpleadoResponse>?, t: Throwable?) {
                            sendCustomEventCatch(t, EventVariableFirebase.EVENT_WS_ADMDES_FAILURE_ALTA_ADMINISTRAR_COMPRA)
                            isLoading.postValue(false)
                            codeError.postValue(600)
                        }

                        override fun onResponse(call: Call<EliminaEmpleadoResponse>?, response: Response<EliminaEmpleadoResponse>?) {
                            if (response!!.isSuccessful) {
                                isLoading.postValue(false)
                                responseLiveData.postValue(response.body())
                                response.body().codigo
                            } else if(response.code() == HttpURLConnection.HTTP_FORBIDDEN) {
                                getEliminaEmpleado(eliminaEmpleadoRequest, responseLiveData, isLoading, codeError)
                            } else {
