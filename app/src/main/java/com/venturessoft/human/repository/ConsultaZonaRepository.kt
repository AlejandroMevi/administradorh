...
class ConsultaZonaRepository : ValidateAndRefreshJWTRepository() {
    ...
    fun getConsultaZonas(consultaZonaRequest: ConsultaZonaRequest, responseLiveData: MutableLiveData<ConsultaZonaResponse>, isLoading: MutableLiveData<Boolean>, codeError:MutableLiveData<Int>) {
    ...
    isLoading.postValue(true)
        ...
        validateAndRefreshJWT(callBackJWTValidateAndRefresh = object : CallBackJWTValidateAndRefresh {
            ...
            override fun onSuccess() {
                ...
                retrofitClient.getConsultaZona(consultaZonaRequest, User.token).enqueue(

                    object : Callback<ConsultaZonaResponse> {
                        override fun onFailure(call: Call<ConsultaZonaResponse>?, t: Throwable?) {
                            sendCustomEventCatch(t, EventVariableFirebase.EVENT_WS_ADMDES_FAILURE_ALTA_ADMINISTRAR_COMPRA)
                            isLoading.postValue(false)
                            codeError.postValue(600)
                        }

                        override fun onResponse(call: Call<ConsultaZonaResponse>?, response: Response<ConsultaZonaResponse>?) {
                            if (response!!.isSuccessful) {
                                isLoading.postValue(false)
                                responseLiveData.postValue(response.body())
                                response.body().codigo
                            } else if(response.code() == HttpURLConnection.HTTP_FORBIDDEN) {
                                getConsultaZonas(consultaZonaRequest, responseLiveData, isLoading, codeError)
                            } else {
