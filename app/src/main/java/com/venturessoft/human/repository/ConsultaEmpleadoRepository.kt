...
class ConsultaEmpleadoRepository : ValidateAndRefreshJWTRepository() {
    ...
    fun getConsultaEmpleado(consultaEmpleadoRequest: ConsultaEmpleadoRequest, responseLiveData: MutableLiveData<ConsultaEmpleadoResponse>, isLoading: MutableLiveData<Boolean>, codeError: MutableLiveData<Int>) {
        ...
        validateAndRefreshJWT(callBackJWTValidateAndRefresh = object : CallBackJWTValidateAndRefresh {
            ...
            override fun onSuccess() {
                ...
                retrofitClient.getConsultaEmpleado(consultaEmpleadoRequest, User.token).enqueue(

                    object : Callback<ConsultaEmpleadoResponse> {

                        override fun onFailure(call: Call<ConsultaEmpleadoResponse>?, t: Throwable?) {
                            sendCustomEventCatch(t, EventVariableFirebase.EVENT_WS_ADMDES_FAILURE_ALTA_ADMINISTRAR_COMPRA)
                            isLoading.postValue(false)
                            codeError.postValue(600)
                        }

                        override fun onResponse(call: Call<ConsultaEmpleadoResponse>?, response: Response<ConsultaEmpleadoResponse>?) {
                            if (response!!.isSuccessful) {
                                isLoading.postValue(false)
                                responseLiveData.postValue(response.body())
                                response.body().codigo
                            } else if(response.code() == HttpURLConnection.HTTP_FORBIDDEN) {
                                getConsultaEmpleado(consultaEmpleadoRequest, responseLiveData, isLoading, codeError)
                            } else {
