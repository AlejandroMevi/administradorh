...
class BusquedaUsuarioRepository : ValidateAndRefreshJWTRepository() {
    ...
    fun getBusquedaUsuario(busquedaUsuarioRequest: BusquedaUsuarioRequest, responseLiveData: MutableLiveData<BusquedaUsuarioResponse>, isLoading: MutableLiveData<Boolean>, codeError: MutableLiveData<Int>) {
        ...
        validateAndRefreshJWT(callBackJWTValidateAndRefresh = object : CallBackJWTValidateAndRefresh {
            ...
            override fun onSuccess() {
                ...
                retrofitClient.getBusquedaUsuario(busquedaUsuarioRequest, User.token).enqueue(

                    object : Callback<BusquedaUsuarioResponse> {

                        override fun onFailure(call: Call<BusquedaUsuarioResponse>?, t: Throwable?) {
                            sendCustomEventCatch(t, EventVariableFirebase.EVENT_WS_ADMDES_FAILURE_ALTA_ADMINISTRAR_COMPRA)
                            isLoading.postValue(false)
                            codeError.postValue(600)
                        }

                        override fun onResponse(call: Call<BusquedaUsuarioResponse>?, response: Response<BusquedaUsuarioResponse>?) {
                            if (response!!.isSuccessful) {
                                isLoading.postValue(false)
                                responseLiveData.postValue(response.body())
                                response.body().codigo
                            } else if(response.code() == HttpURLConnection.HTTP_FORBIDDEN) {
                                getBusquedaUsuario(busquedaUsuarioRequest, responseLiveData, isLoading, codeError)
                            } else {
