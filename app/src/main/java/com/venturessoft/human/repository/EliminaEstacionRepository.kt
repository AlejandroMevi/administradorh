...
class EliminaEstacionRepository : ValidateAndRefreshJWTRepository() {
    ...
    fun getEliminaEstacion(eliminaEstacionRequest: EliminaEstacionRequest, responseLiveData: MutableLiveData<EliminaEstacionResponse>, isLoading: MutableLiveData<Boolean>, codeError: MutableLiveData<Int>) {
        ...
        validateAndRefreshJWT(callBackJWTValidateAndRefresh = object : CallBackJWTValidateAndRefresh {
            ...
            override fun onSuccess() {
                retrofitClient.getEliminaEstacion(eliminaEstacionRequest, User.token).enqueue(

                    object : Callback<EliminaEstacionResponse> {
                        override fun onFailure(call: Call<EliminaEstacionResponse>?, t: Throwable?) {
                            sendCustomEventCatch(t, EventVariableFirebase.EVENT_WS_ADMDES_FAILURE_ALTA_ADMINISTRAR_COMPRA)
                            isLoading.postValue(false)
                            codeError.postValue(600)
                        }

                        override fun onResponse(call: Call<EliminaEstacionResponse>?, response: Response<EliminaEstacionResponse>?) {
                            if (response!!.isSuccessful) {
                                isLoading.postValue(false)
                                responseLiveData.postValue(response.body())
                                response.body().codigo
                            } else if(response.code() == HttpURLConnection.HTTP_FORBIDDEN) {
                                getEliminaEstacion(eliminaEstacionRequest, responseLiveData, isLoading, codeError)
                            }
                            else {
