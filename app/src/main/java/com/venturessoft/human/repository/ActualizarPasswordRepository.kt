...
class ActualizarPasswordRepository : ValidateAndRefreshJWTRepository() {
    ...
    fun getActualizarPassword(actualizarPasswordRequest: ActualizarPasswordRequest, responseLiveData: MutableLiveData<ActualizarPasswordResponse>, isLoading: MutableLiveData<Boolean>, codeError: MutableLiveData<Int>) {
        isLoading.postValue(true)
        validateAndRefreshJWT(callBackJWTValidateAndRefresh = object :
            CallBackJWTValidateAndRefresh {
            override fun onSuccess() {
                retrofitClient.getActualizarPassword(actualizarPasswordRequest, User.token).enqueue(

                    object : Callback<ActualizarPasswordResponse> {
                        override fun onFailure(call: Call<ActualizarPasswordResponse>?, t: Throwable?) {
                            sendCustomEventCatch(t, EventVariableFirebase.EVENT_WS_ADMDES_FAILURE_ALTA_ADMINISTRAR_COMPRA)
                            isLoading.postValue(false)
                            codeError.postValue(600)
                        }

                        override fun onResponse(call: Call<ActualizarPasswordResponse>?, response: Response<ActualizarPasswordResponse>?) {
                            if (response!!.isSuccessful) {
                                isLoading.postValue(false)
                                responseLiveData.postValue(response.body())
                                response.body().codigo
                            } else if (response.code() == HttpURLConnection.HTTP_FORBIDDEN) {
                                getActualizarPassword(actualizarPasswordRequest, responseLiveData, isLoading, codeError)
                            } else {
