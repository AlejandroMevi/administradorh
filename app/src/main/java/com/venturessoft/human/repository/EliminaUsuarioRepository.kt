...
class EliminaUsuarioRepository : ValidateAndRefreshJWTRepository() {
    ...
    fun getEliminaUsuario(eliminaUsuarioRequest: EliminaUsuarioRequest, responseLiveData: MutableLiveData<EliminaUsuarioResponse>, isLoading: MutableLiveData<Boolean>, codeError: MutableLiveData<Int>) {
        ...
        validateAndRefreshJWT(callBackJWTValidateAndRefresh = object : CallBackJWTValidateAndRefresh {
            ...
            override fun onSuccess() {
                ...
                retrofitClient.getEliminaUsuarioSupervisor(eliminaUsuarioRequest, User.token).enqueue(

                    object : Callback<EliminaUsuarioResponse> {
                        override fun onFailure(call: Call<EliminaUsuarioResponse>?, t: Throwable?) {
                            sendCustomEventCatch(t, EventVariableFirebase.EVENT_WS_ADMDES_FAILURE_ALTA_ADMINISTRAR_COMPRA)
                            isLoading.postValue(false)
                            codeError.postValue(600)
                        }

                        override fun onResponse(call: Call<EliminaUsuarioResponse>?, response: Response<EliminaUsuarioResponse>?) {
                            if (response!!.isSuccessful) {
                                isLoading.postValue(false)
                                responseLiveData.postValue(response.body())
                                response.body().codigo
                            } else if(response.code() == HttpURLConnection.HTTP_FORBIDDEN) {
                                getEliminaUsuario(eliminaUsuarioRequest, responseLiveData, isLoading, codeError)
                            } else {
