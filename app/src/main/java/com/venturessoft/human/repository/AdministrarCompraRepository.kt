...
class AdministrarCompraRepository : ValidateAndRefreshJWTRepository() {
    ...
    fun getAdministrarCompra(administrarCompraRequest: AdministrarCompraRequest, responseLiveData: MutableLiveData<AdministrarCompraResponse>, isLoading: MutableLiveData<Boolean>, codeError: MutableLiveData<Int>) {
       ...
       isLoading.postValue(true)
        ...
        validateAndRefreshJWT(callBackJWTValidateAndRefresh = object : CallBackJWTValidateAndRefresh {
            override fun onSuccess() {
                ...
                retrofitClient.getAdministrarCompra(administrarCompraRequest, User.token).enqueue(

                    object : Callback<AdministrarCompraResponse> {

                        override fun onFailure(call: Call<AdministrarCompraResponse>?, t: Throwable?) {
                            sendCustomEventCatch(t, EventVariableFirebase.EVENT_WS_ADMDES_FAILURE_ALTA_ADMINISTRAR_COMPRA)
                            isLoading.postValue(false)
                            codeError.postValue(600)
                        }

                        override fun onResponse(call: Call<AdministrarCompraResponse>?, response: Response<AdministrarCompraResponse>?) {
                            if (response!!.isSuccessful) {
                                isLoading.postValue(false)
                                responseLiveData.postValue(response.body())
                                response.body().codigo
                            } else if(response.code() == HttpURLConnection.HTTP_FORBIDDEN) {
                                getAdministrarCompra(administrarCompraRequest, responseLiveData, isLoading, codeError)
                            }
                            else {
